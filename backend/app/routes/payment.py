...
class VerifyRequest(BaseModel):
    ...
    cart_item_id: int = None  # (Optional) Link to cart item to clear it
...
def verify_payment(data: VerifyRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # 3. If this was from a cart, remove the cart item (Permanently deduct)
        if data.cart_item_id:
            from app.models import CartItem
            cart_item = db.query(CartItem).filter(CartItem.id == data.cart_item_id).first()
            if cart_item:
                db.delete(cart_item)
                print(f"[Cart] Item {data.cart_item_id} permanently removed after purchase.")
...
@router.get("/admin/transactions/{admin_id}")
def get_admin_transactions(admin_id: int, db: Session = Depends(get_db)):
    from app.models import Transaction, Product
    try:
        return db.query(Transaction).join(Product, Transaction.product_name == Product.name).filter(Product.admin_id == admin_id).order_by(Transaction.created_at.desc()).all()
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
